...
cliente.listar = async(req, res) => {
    ...
    const lista = await sql.query("SELECT * FROM info_cliente WHERE idInfo_cliente=?", [usuario])
    res.render("cliente/listar", {lista});
}

cliente.traer = async(req, res) => {
    const id  = req.params.id
    const trae = await sql.query("SELECT * FROM info_cliente WHERE idInfo_cliente=?", [id])
    console.log(trae)
